...
bool isAlphaU(char c) { return (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') || c == '_'; }
bool isAlnumU(char c) { return isAlphaU(c) || (c >= '0' && c <= '9'); }
bool isDigit(char c)  { return c >= '0' && c <= '9'; }
bool isSpace(char c)  { return c == ' ' || c == '\t' || c == '\n' || c == '\r' || c == '\f' || c == '\v'; }

// ── Tokens ────────────────────────────────────────────────────────────────────
enum class Kind { Data, Action, Unary, RParen };
struct Token { std::string val; Kind kind; };
...
std::vector<Token> tokenize(const std::string& expr) {
  ...
  while (i < n) {
    ...
    if (c == ')') { tokens.push_back({")", Kind::RParen}); i++; continue; }
    i++;
  }
  return tokens;
}
...
// ── Pair stack ────────────────────────────────────────────────────────────────
struct Pair { bool hasData; std::string data; std::string op; };
...
Result runStackAlgorithm(const std::string& expr, int startTemp = 0) {
  ...
  auto popAndGenerate = [&](const std::string& rightData) -> std::string {
    Pair top = stack.back(); stack.pop_back();
  ...
  auto pushOp = [&](const std::string& op) {
    ...
    if (op == "(") {
      ...
      step();
      hasPending = false; pendingData.clear();
      return;
    }
    while (!stack.empty() && stack.back().op != "(" &&
           stackPriority(stack.back().op) >= prIn) {
      std::string t = popAndGenerate(pendingData);
      step();
      hasPending = true; pendingData = t;
    }
  ...
  while (i < tokens.size()) {
    ...
    if (tok.kind == Kind::Data) {
      hasPending = true; pendingData = tok.val;
      if (!nextTok || nextTok->kind == Kind::RParen) { step(); i++; }
      else if (nextTok->kind == Kind::Action)        { pushOp(nextTok->val); i += 2; }
      else if (nextTok->kind == Kind::Unary)         { step(); i++; }
      else                                           { i++; }
    } else if (tok.kind == Kind::Unary) {
      pushOp("u" + tok.val); i++;
    } else if (tok.kind == Kind::Action) {
      if (tok.val == "(") pushOp("("); else pushOp(tok.val);
      i++;
      ...
      if (!stack.empty()) { stack.pop_back(); step(); }
      i++;
    } else { i++; }
  }

  while (!stack.empty()) {
    if (stack.back().op == "(") { stack.pop_back(); break; }
    std::string t = popAndGenerate(pendingData);
    step();
    hasPending = true; pendingData = t;
  }
...
struct Analysis { std::string expr; Result r; };
...
std::vector<Analysis> analyzeExpressions(const std::string& actionsStr, int startTemp = 0) {
  ...
  while (std::getline(ss, assign, ';')) {
    ...
    bool hasOp = false;
    for (char c : rhs) if (c=='+'||c=='-'||c=='*'||c=='/'||c=='('||c==')') { hasOp = true; break; }
    if (!hasOp) continue;
